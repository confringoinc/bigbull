...
class Dashboard extends React.Component {
			...
			fetch(percentageChange)
				...
				.then((result) => {
					if (result.latestPrice !== null) {
						priceStash[parseInt(num)] = result.latestPrice.toFixed(
							2
						);
					} else if (result.iexRealtimePrice !== null) {
						priceStash[
							parseInt(num)
						] = result.iexRealtimePrice.toFixed(2);
					}
	...
	getStocksList() {
		const stocks = `https://cloud.iexapis.com/stable/stock/market/list/mostactive?token=${process.env.REACT_APP_IEX_KEY_1}`;
		fetch(stocks)
			.then((res) => res.json())
			.then((result) => {
				const gainers = `https://cloud.iexapis.com/stable/stock/market/list/gainers?token=${process.env.REACT_APP_IEX_KEY_1}`;
				let counter = 0;
				fetch(gainers)
					.then((res) => res.json())
					.then((result) => {
						for (let i = 0; i < result.length; i++) {
							if (result[parseInt(i)].latestPrice !== null) {
								tempStocksSymbols.push(
									result[parseInt(i)].symbol
								);
								tempStockName.push(
									result[parseInt(i)].companyName
								);
								tempStockPrice.push(
									`$${result[parseInt(i)].latestPrice.toFixed(
										2
									)}`
								);
							}
						...
						for (let i = 0; i < 9; i++) {
							if (typeof result[parseInt(i)] !== "undefined") {
								if (
									...
									stockList[parseInt(i)] =
										tempStockName[parseInt(counter)];
	...
	getLatestPrice(symbol, i) {
		const lastPrice = `https://cloud.iexapis.com/stable/stock/${symbol}/quote?displayPercent=true&token=${process.env.REACT_APP_IEX_KEY_1}`;
		fetch(lastPrice)
			.then((res) => res.json())
			.then((result) => {
				portfolioValue[parseInt(i)] = parseFloat(
					Number(
						portfolioShares[parseInt(i)] * result.latestPrice
					).toFixed(2)
				);
			})
			.then(() => {
		...
		for (let i = 0; i < stockSymbols.length; i++) {
			if (Math.sign(stockChanges[parseInt(i)]) === -1) {
				...
				if (
					...
					stockChanges[parseInt(i)] = stockChanges[
						parseInt(i)
					].substr(1);
				}
			} else {
			...
			if (
				document.getElementById("searchBar") === document.activeElement
			) {
									...
									>
										...
										{this.state.loader3 === false && (
											<div className="errorMsg">
												...
												<p>
													Couldn't load list try again in few
													minutes
