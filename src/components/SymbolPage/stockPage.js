...
export default class stockPage extends React.Component {
	...
	componentDidMount() {
		...
		let data = Firebase.database().ref("value/");
		data.once("value", (snapshot) => {
			const valref = snapshot.val();
			valref && valref.map((data) => (
				symbolsOnly.push(data.Symbol)
			));
		}).then(() => {
			symbol = window.location.href.split("/")[
				window.location.href.split("/").length - 1
			...
			<section className="stock">
				...
				{this.state.valid === false && (
					<div className="wrongSymbol">
						...
						<h2>OR</h2>
						<h3>
							Go to <Link to="/bigbull/dashboard">Dashboard</Link>
						</h3>
					</div>
				)}
			</section>
